	...
	FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	// Debuff
	DetermineDebuff(Spec, ExecutionParams, EvalParams, TagsToCaptureDefs);
	...
	for (const TTuple<FGameplayTag, FGameplayTag>& Pair : FAuraGameplayTags::Get().DamageTypesToResistances)
	{
		...
		if (UAuraAbilitySystemLibrary::IsRadialDamage(EffectContextHandle))
		{
			...
			// 4. Call ApplyRadialDamageWithFalloff to cause damage (this will result in TakeDamage being called on the victim, which will broadcast OnDamageDelegate)
...
void UExecCalc_Damage::DetermineDebuff(const FGameplayEffectSpec& Spec, const FGameplayEffectCustomExecutionParameters& ExecutionParams, FAggregatorEvaluateParameters& EvalParams, const TMap<FGameplayTag, FGameplayEffectAttributeCaptureDefinition>& InTagsToCaptureDefs) const
{
	const FAuraGameplayTags& GameplayTags = FAuraGameplayTags::Get();
	for (TTuple<FGameplayTag, FGameplayTag> Pair : GameplayTags.DamageTypesToDebuffs)
	{
		...
		if (TypeDamage > -.5f) // .5 padding for floating point imprecision
		{
			...
			ExecutionParams.AttemptCalculateCapturedAttributeMagnitude(InTagsToCaptureDefs[ResistanceTag], EvalParams, TargetDebuffResistance);
